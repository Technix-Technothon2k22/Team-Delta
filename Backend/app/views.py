...
def getData(request):
    try:
        ...
        abc, _ = UserDataModel.objects.get_or_create(user=user)
        if ser.is_valid():
...
def analyseData(request):
    try:
        ...
        authentication_classes = [JWTAuthentication]
        permission_classes = [IsAuthenticated]
        user_data_obj, _ = UserDataModel.objects.get_or_create(user = UserModel.objects.get(email=request.user.email))
        ...
        user_data_obj.save()
        return Response({"messgae":"Analysing Results"}, status=status.HTTP_200_OK)
    except Exception as e:
        print(e)
        return Response({"error":str(e), "message":"Something went wrong"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def fetchResults(request):
    try:
        ...
        permission_classes = [IsAuthenticated]
        user = UserModel.objects.get(email=request.user.email)
        user_data_obj = UserDataModel.objects.get(user=user)
